...
describe("parseAndValidateRequestValue", () => {
  ...
  it("should throw an error when value is undefined", () => {
    expect(() =>
      parseAndValidateRequestValue(undefined, "srcLanguage")
    ).toThrow(Error);
  });

  it("should throw an error when value is not a valid number", () => {
    expect(() => parseAndValidateRequestValue("abc", "srcLanguage")).toThrow(
      Error
    );
  });

  it("should throw an error when value is 0 or negative number", () => {
    expect(() => parseAndValidateRequestValue("0", "srcLanguage")).toThrow(
      Error
    );
    expect(() => parseAndValidateRequestValue("-1", "srcLanguage")).toThrow(
      Error
    );
  });
