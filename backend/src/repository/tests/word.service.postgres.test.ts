...
} from "../items.repository.postgres";
...
describe("getWordsPostgres tests", () => {
  ...
  it("should return specific words for a user", async () => {
    const words = await getItemsRepository(
      postgresDBPool,
      userId,
      languageID,
      numWords
  ...
  it("should throw error for nonexistent user", async () => {
    await expect(
      getItemsRepository(postgresDBPool, 999, languageID, numWords)
...
describe("updateWordsPostgres tests", () => {
  ...
  it("should throw Error on update for a nonexistent user_id", async () => {
    await expect(
      patchItemsRepository(postgresDBPool, 999, wordToUpdateValid)
  ...
  it("should throw Error on update for a nonexistent word_id", async () => {
    ...
    await expect(
      patchItemsRepository(postgresDBPool, userId, wordToUpdateInvalid)
    ).rejects.toThrowError(Error);
  });

  it("should create new user_word", async () => {
    await patchItemsRepository(postgresDBPool, userId, wordToUpdateValid);
  ...
  it("should update user_word progress with valid progress range", async () => {
    ...
    await patchItemsRepository(postgresDBPool, userId, wordToUpdateNew);
  ...
  it("should update multiple words in a single call", async () => {
    ...
    await patchItemsRepository(postgresDBPool, userId, wordsToUpdate);
  ...
  it("should mark a word as learned if progress is equal learnedAt limit", async () => {
    ...
    await patchItemsRepository(postgresDBPool, userId, wordToUpdate);
