...
async def prepare_english_words(file_name: str, output_folder: str, audio_folder: str, word_number: int) -> None:
    ...
    df = pd.read_csv(file_name, usecols=[0], nrows=word_number, names=["trg"], header=None)
    ...
    output_file = os.path.join(output_folder, os.path.basename("CZ_EN.csv"))
    await async_save_csv(final_df, output_file)
    print(f"Saved processed CSV: {"CZ_EN.csv"}")

if __name__ == "__main__":

    input_file = os.path.abspath("data/en-source/20k_en_words.csv")  
